"""
=============================================================================
train_model3_emotion_shift.py
=============================================================================
Model 3 — INDEPENDENT TRAINING
Dataset : emotion_shift.csv  (10,512 pairs)
Base    : roberta-base  (fresh — no dependency on any other model)
Output  : /kaggle/working/model_emotion_shift/

What this dataset contains:
  - Previous and current message have DIFFERENT emotions (emotion shift)
  - Example: previous="I am happy!", current="But now I'm worried..."
  - Emotion label = current message emotion
  - Teaches model: detecting emotion change given contrasting context
  - Most balanced dataset (all emotions nearly equal ~880 each)
=============================================================================
"""
...
# CONFIG
# =============================================================================
CSV_PATH   = "/kaggle/input/datasets/kakulthisside/context-dataset/emotion_shift.csv"
BASE_MODEL = "roberta-base"                        # ✅ Fresh start — no dependency
OUTPUT_DIR = "/kaggle/working/model_emotion_shift"

MAX_LEN      = 128
BATCH_SIZE   = 32
EPOCHS       = 6          # More epochs — smaller dataset needs more passes
LR           = 2e-5
WARMUP_RATIO = 0.1
PATIENCE     = 3          # More patience — smallest dataset
DEVICE       = torch.device("cuda" if torch.cuda.is_available() else "cpu")
USE_AMP      = DEVICE.type == "cuda"

print("=" * 55)
print("  MODEL 3 — emotion_shift.csv")
...
print(f"\n   ℹ️  Note: This is the MOST BALANCED dataset (~880 samples per emotion)")
print(f"   In all pairs: previous emotion ≠ current emotion")
...
print(f"\n⚖️  Imbalance ratio: {weights.max()/weights.min():.1f}x  (most balanced!)")
...
print(f"\n{'='*55}")
print(f"  ✅ MODEL 3 COMPLETE — emotion_shift")
